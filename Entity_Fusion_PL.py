...
class SimilarityMatrixGeneratorPolars:
    ...
    def cluster_data(
        ...
        # Inject must-links from old labels if provided
        ...
        # Compute edges per nested condition tree
        ...
        if old_label_map is not None:
            ...
            label_df = pl.DataFrame(
                {
                    self.id_col: list(node_to_label.keys()),
                    "cluster_label": list(node_to_label.values()),
                }
            )
        else:
            ...
            label_df = pl.DataFrame(
                {
                    self.id_col: list(self.clusters.keys()),
                    "cluster_label": list(self.clusters.values()),
                }
            )

        # Join back; assign unique labels for rows not in any component
        out = self.df.join(label_df, on=self.id_col, how="left")

        # Assign unique labels to rows still unlabeled, without zip_with shape issues
        null_total = out.select(
            pl.col("cluster_label").is_null().sum().alias("n")
        ).item()
        if null_total > 0:
            ...
            out = (
                ...
                .with_columns(
                    pl.coalesce([pl.col("cluster_label"), pl.col("__fill__")]).alias(
                        "cluster_label"
                    )
                )
